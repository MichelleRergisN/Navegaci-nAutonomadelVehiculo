...
class TurtleController(Node):
    def __init__(self):
        ...
        self.get_logger().info("Controlador de Lazo Cerrado Iniciado [turtlesim mode]")

        # CAMBIO clave: turtlesim escucha /turtle1/cmd_vel (no /cmd_vel)
        ...
        self.Kw                 = 1.2
        self.Kv                 = 0.5
        self.tolerance_distance = 0.05   # un poco más holgado para turtlesim
    ...
    def target_callback(self, msg):
        if not self.got_target:
            ...
            self.state      = "state1"
            self.get_logger().info(f"Nuevo objetivo: ({msg.x:.2f}, {msg.y:.2f})")
    ...
    def state_machine(self):
        if self.state == "state1":
            ...
            if self.end_of_accion:
                self.state         = "stop"
                self.end_of_accion = False
                self.got_target    = False   # listo para el siguiente punto
        elif self.state == "stop":
            self.stop_robot()

    def go_to_angle(self):
        msg = Twist()
        e_theta = self._angle_error()
